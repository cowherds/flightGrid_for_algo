...
@AlgorithmRegistry.register_scheduler("distance_first")
class DistanceFirstScheduler(InsertionScheduler):
    def plan(self, current_drones: List[Drone], unassigned_tasks: List[Task]) -> Dict[str, List[Task]]:
        ...
        ref_point = np.array(list(self.depots.values())[0].as_tuple())
        sorted_tasks = sorted(
            unassigned_tasks,
            key=lambda task: (-task.priority, -np.linalg.norm(np.array(task.location.as_tuple()) - ref_point)),
        )
        ...
        for task in sorted_tasks:
            ...
            preferred_task_ids = self._build_preferred_task_sets(current_drones, routes, [task])
